...
if home_team != away_team:
    if st.button("Execute", type="primary"):
        ...
        # 50% Season / 50% Last 10 Curve Combination
        ...
        # --- THE DYNAMIC NBA HCA CONDITIONAL OVERRIDE ---
        # Checks if league selector is NBA. If yes -> 0.0 HCA, else -> 3.5 points HCA
        active_hca = 0.0 if selected_league == "NBA" else GLOBAL_HCA_POINTS
        
        calculated_final_home = base_final_home + active_hca
        ...
        # Log-5 Regression Distribution modeling for Win Probability calculation
        home_net = home_blended_ortg - home_blended_drtg
        away_net = away_blended_ortg - away_blended_drtg
        efficiency_margin = home_net - away_net + (active_hca * (100 / projected_possessions))
        ...
        with st.expander("🔬 View Advanced Diagnostics"):
            ...
            with col_diag1:
                ...
                st.markdown(f"**{home_team} Baseline Score (Neutral):** {base_final_home:.2f}")
            with col_diag2:
                st.markdown(f"**Applied League Score Bonus:** +{active_hca} PTS")
