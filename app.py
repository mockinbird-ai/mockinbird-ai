...
selected_league = st.sidebar.selectbox(
    ...
    [
        "NBA", "Spain: Liga ACB", "France: LNB Élite", "Germany: easyCredit BBL", 
...
# REMOVED RAW TABLE TO MATCH VISUAL DEMAND IN 1000635650.jpg
...
if team_a != team_b:
    if st.button("Execute Deep Analytical Prediction", type="primary"):
        ...
        # Deeper analysis variables calculation
        ...
        # Score generation algorithm based on advanced variables
        raw_final_a = (team_a_exp_off * projected_possessions) / 100
        raw_final_b = (team_b_exp_off * projected_possessions) / 100
        
        # Include baseline standard home court advantage adjustment
        raw_final_a += 2.5 
        
        final_score_a = int(np.round(raw_final_a))
        final_score_b = int(np.round(raw_final_b))
        
        # Generate variance factors for half-time allocations
        half_score_a = int(np.round(final_score_a * np.random.uniform(0.46, 0.51)))
        half_score_b = int(np.round(final_score_b * np.random.uniform(0.46, 0.51)))
        ...
        st.markdown("---")
        st.header(f"🦅 Predicted Winner: {winner}")
        st.metric("Model Confidence Score", f"{conf:.2f}%")
        
        # Score display matrix
        st.subheader("📋 Projected Scoreboard Analysis")
    ...
    st.warning("Please select two unique teams to execute simulation parameters.")
