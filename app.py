...
# Page Configuration
st.set_page_config(page_title="MiHoops Pro Analytics", page_icon="🏀", layout="centered")
st.title("🏀 MiHoops Global Analytics Prediction Engine")
st.markdown("### Multi-League Advanced Efficiency Projections")
st.markdown("---")

# =========================================================================
# GLOBAL LEAGUE REGISTRY & HISTORICAL BASELINE BENCHMARKS
# Sourced via Basketball-Reference.com & RealGM.com Advanced Tracker
# =========================================================================
GLOBAL_LEAGUE_DB = {
    "NBA": {
        "avg_pace": 99.2, "avg_ortg": 115.6, "hca_bonus": 2.1, "source": "Basketball-Reference.com",
        "teams": {
    ...
    "WNBA": {
        "avg_pace": 80.8, "avg_ortg": 103.5, "hca_bonus": 2.1, "source": "Basketball-Reference.com",
        "teams": {
    ...
    "Spain (Liga ACB)": {
        "avg_pace": 76.4, "avg_ortg": 112.1, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "France (LNB Élite)": {
        "avg_pace": 75.8, "avg_ortg": 110.5, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "England (Super League Basketball)": {
        "avg_pace": 79.5, "avg_ortg": 106.8, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Germany (easyCredit BBL)": {
        "avg_pace": 78.2, "avg_ortg": 111.2, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Italy (Lega Basket Serie A)": {
        "avg_pace": 76.1, "avg_ortg": 111.8, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Portugal (Liga Betclic)": {
        "avg_pace": 77.9, "avg_ortg": 107.5, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Türkiye (BSL)": {
        "avg_pace": 77.2, "avg_ortg": 113.4, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Puerto Rico (BSN)": {
        "avg_pace": 83.5, "avg_ortg": 113.2, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "New Zealand (NBL)": {
        "avg_pace": 84.1, "avg_ortg": 109.8, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Greece (GBL)": {
        "avg_pace": 74.2, "avg_ortg": 110.1, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Japan (B.League)": {
        "avg_pace": 75.1, "avg_ortg": 108.4, "hca_bonus": 1.5, "source": "RealGM.com",
        "teams": {
    ...
    "China (CBA)": {
        "avg_pace": 88.5, "avg_ortg": 111.2, "hca_bonus": 2.2, "source": "RealGM.com",
        "teams": {
    ...
    "Canada (CEBL)": {
        "avg_pace": 82.8, "avg_ortg": 111.4, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Austria (Superliga)": {
        "avg_pace": 76.2, "avg_ortg": 105.4, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Czech Republic (NBL)": {
        "avg_pace": 77.4, "avg_ortg": 108.1, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Israel (Winner League)": {
        "avg_pace": 78.0, "avg_ortg": 112.5, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
    ...
    "Belgium (BNXT League)": {
        "avg_pace": 74.9, "avg_ortg": 106.2, "hca_bonus": 1.8, "source": "RealGM.com",
        "teams": {
...
# =========================================================================
# UI REGION: LEAGUE & TEAM SELECTION
# =========================================================================
selected_league = st.sidebar.selectbox("🏀 Select Active Competition League", list(GLOBAL_LEAGUE_DB.keys()))
league_cfg = GLOBAL_LEAGUE_DB[selected_league]

st.subheader(f"🏟️ Matchup Variant Setup: {selected_league}")
st.sidebar.caption(f"Data Pipeline Lineage: {league_cfg['source']}")

col1, col2 = st.columns(2)
with col1:
    home_team = st.selectbox("🏡 Select Home Team Unit", sorted(league_cfg["teams"].keys()), index=0)
with col2:
    away_team = st.selectbox("✈️ Select Away Team Unit", sorted(league_cfg["teams"].keys()), index=1 if len(league_cfg["teams"]) > 1 else 0)

if home_team == away_team:
    st.error("⚠️ Setup Error: Select two unique matching teams for analysis.")
else:
    # Extract Team Metrics
    h_stats = league_cfg["teams"][home_team]
    a_stats = league_cfg["teams"][away_team]
    
    # -------------------------------------------------------------------------
    # CORE MATH: LOG-RATIO RELATIVE NET EFFICIENCY ENGINE
    # -------------------------------------------------------------------------
    # Proportional Pace Adjustment
    predicted_pace = (h_stats["PACE"] * a_stats["PACE"]) / league_cfg["avg_pace"]
    
    # Defensive Suppression Regression Metrics
    home_expected_efficiency = (h_stats["ORTG"] * a_stats["DRTG"]) / league_cfg["avg_ortg"]
    away_expected_efficiency = (a_stats["ORTG"] * h_stats["DRTG"]) / league_cfg["avg_ortg"]
    
    # Absolute Output Scoring (HCA Applied Only Here As A Baseline Adjuster)
    projected_home_ft = (home_expected_efficiency * (predicted_pace / 100)) + league_cfg["hca_bonus"]
    projected_away_ft = (away_expected_efficiency * (predicted_pace / 100))
    
    # Margins and Line Values
    raw_spread = projected_away_ft - projected_home_ft
    winner = home_team if projected_home_ft > projected_away_ft else away_team
    winner_icon = "🏡" if winner == home_team else "✈️"

    # -------------------------------------------------------------------------
    # UI DISPLAY: WINNER BLOCKS
    # -------------------------------------------------------------------------
    st.markdown("## 🏆 Consensus Analytical Winner Pick")
    with st.container():
        m1, m2 = st.columns(2)
        with m1:
            st.metric(label="Outright Winner Selection", value=f"{winner_icon} {winner}")
        with m2:
            st.metric(label="Calculated Model Spread Line", value=f"{winner} {abs(raw_spread):-.1f}")
            
    st.markdown("---")

    # -------------------------------------------------------------------------
    # UI DISPLAY: PERFORMANCE MATRIX SUMMARY TABLE
    # -------------------------------------------------------------------------
    ...
    matrix_summary_df = pd.DataFrame({
        ...
        "Half Time (HT) Projection": [f"{projected_home_ft * 0.5:.1f}", f"{projected_away_ft * 0.5:.1f}"],
        "Final Score (FT) Projection": [f"{projected_home_ft:.1f}", f"{projected_away_ft:.1f}"],
        "Model Point Spread Line": [f"{raw_spread:+.1f}", f"{-raw_spread:+.1f}"]
    })
    
    st.table(matrix_summary_df)

    # -------------------------------------------------------------------------
    # UI DISPLAY: ADVANCED TECHNICAL EXPANDER
    # -------------------------------------------------------------------------
    with st.expander("🔍 View Raw Advanced Analytical Variables Used"):
        st.markdown(f"##### League Infrastructure Baseline Data ({league_cfg['source']})")
        l_col1, l_col2 = st.columns(2)
        with l_col1:
            st.markdown(f"* **League Pace Average:** `{league_cfg['avg_pace']}` possessions")
            st.markdown(f"* **League Offensive Average (ORTG):** `{league_cfg['avg_ortg']}` points/100 poss")
        with l_col2:
            st.markdown(f"* **Applied Home Court Advantage (HCA):** `+{league_cfg['hca_bonus']}` points")
            st.markdown(f"* **Simulated Matchup Pace Line:** `{predicted_pace:.2f}` possessions")
            
        st.markdown("---")
        st.markdown("##### Team Advanced Metrics Dataset Breakdown")
        
        t_col1, t_col2 = st.columns(2)
        with t_col1:
            st.markdown(f"**🏡 {home_team}**")
            st.markdown(f"* Team Total Offensive Rating: `{h_stats['ORTG']}`")
            st.markdown(f"* Team Defensive Rating: `{h_stats['DRTG']}`")
            st.markdown(f"* Baseline Adjusted Efficiency Expectancy: `{home_expected_efficiency:.2f}`")
        with t_col2:
            st.markdown(f"**✈️ {away_team}**")
            st.markdown(f"* Team Total Offensive Rating: `{a_stats['ORTG']}`")
            st.markdown(f"* Team Defensive Rating: `{a_stats['DRTG']}`")
            st.markdown(f"* Baseline Adjusted Efficiency Expectancy: `{away_expected_efficiency:.2f}`")
